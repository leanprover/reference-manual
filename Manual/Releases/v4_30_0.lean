...
Lean 4.30.0 brings a new `sym =>` interactive tactic mode, a significantly expanded {tactic}`cbv` tactic, user-controllable borrow annotations with completion of the new LCNF compiler backend, and a comprehensive overhaul of Lake's caching infrastructure.

## New `sym =>` Interactive Tactic

[#12970](https://github.com/leanprover/lean4/pull/12970) adds `sym =>`, an interactive tactic mode built on {tactic}`grind`. Unlike `grind =>`, it gives users explicit control over each step — `intro`, `apply`, `internalize`, `by_contra`, and `simp` — without eagerly applying by-contradiction. Satellite solvers like `lia` and `ring` handle remaining binders automatically when invoked. The mode is backed by a new `Sym.simp` simplifier that also powers {tactic}`cbv` and {tactic}`grind`, with support for named variants ([#13034](https://github.com/leanprover/lean4/pull/13034)), named theorem sets ([#13018](https://github.com/leanprover/lean4/pull/13018)), and loop prevention for permutation theorems ([#13046](https://github.com/leanprover/lean4/pull/13046)).

## `cbv` Tactic Expansion

The {tactic}`cbv` tactic receives major new capabilities: [#12597](https://github.com/leanprover/lean4/pull/12597) adds a `cbv_simproc` system for user-defined simplification procedures (mirroring {tactic}`simp`'s `simproc` infrastructure); [#12773](https://github.com/leanprover/lean4/pull/12773) adds `at` location syntax (`cbv at h`, `cbv at *`); [#12763](https://github.com/leanprover/lean4/pull/12763) adds short-circuit evaluation for `Or`/`And`; and [#12788](https://github.com/leanprover/lean4/pull/12788) adds `set_option cbv.maxSteps N`.

## Compiler: User Borrow Annotations and New LCNF Backend

[#12830](https://github.com/leanprover/lean4/pull/12830) enables user-provided borrow annotations: mark arguments with `(x : @&Ty)` to reduce reference counting pressure. Use `trace.Compiler.inferBorrow` to inspect the compiler's reasoning. [#12942](https://github.com/leanprover/lean4/pull/12942) marks `ReaderT`'s context argument as borrowed, propagating RC savings across the metaprogramming stack.

[#12781](https://github.com/leanprover/lean4/pull/12781) ports the C emission pass to LCNF, completing end-to-end code generation through the new infrastructure. [#12665](https://github.com/leanprover/lean4/pull/12665) ports the reset/reuse pass with improved exponential-code prevention, yielding a *~15% decrease in binary size*.

## Lake Cache Overhaul

[#12634](https://github.com/leanprover/lean4/pull/12634) enables on-demand artifact downloads during `lake build`, removing the need for a separate `lake cache get` step. [#12974](https://github.com/leanprover/lean4/pull/12974) adds parallel transfers via `curl --parallel`, and [#13164](https://github.com/leanprover/lean4/pull/13164) fetches all artifact URLs from Reservoir in a single bulk request. [#13144](https://github.com/leanprover/lean4/pull/13144) adds staged upload commands (`lake cache stage`/`unstage`/`put-staged`) paralleling Mathlib's `lake exe cache` workflow.

## Theorems Are Now Opaque in the Kernel

[#12973](https://github.com/leanprover/lean4/pull/12973) makes theorems opaque to the kernel: a `theorem` is never unfolded during reduction or type checking, closing a gap that proof irrelevance had already made largely theoretical. Proofs that must reduce (e.g., `Acc.rec` eliminating into `Type`) must use `def` instead.

## `@[deprecated_arg]` Attribute

[#13011](https://github.com/leanprover/lean4/pull/13011) adds `@[deprecated_arg old new (since := “...”)]` for deprecating individual function parameters. Callers using the old name receive a warning and a rename code action; removed parameters produce an error with a delete hint.

## Library Highlights

[#12126](https://github.com/leanprover/lean4/pull/12126)–[#12144](https://github.com/leanprover/lean4/pull/12144) introduce core HTTP data types (`Request`, `Response`, `Status`, `Headers`, `URI`, streaming `Body`) as the foundation of a standard HTTP library. String verification continues from v4.29.0, adding proofs for `startsWith`, `split`, `intercalate`, `isNat`/`toNat?`, `isInt`/`toInt?`, and more. [#12385](https://github.com/leanprover/lean4/pull/12385) adds `Array.mergeSort`, a stable O(n log n) sort about twice as fast as `List.mergeSort` on large random inputs.

## Experimental: Live Debugging with `idbg`

[#12648](https://github.com/leanprover/lean4/pull/12648) adds experimental `idbg e` syntax: when placed in a `do` block, it connects a running compiled program to the language server over TCP and evaluates `e` with actual runtime values. Editing the expression re-evaluates it live. Known limitations: single `idbg` at a time, requires `LEAN_PATH`, untested on Windows/macOS.

## Breaking Changes

- [#12973](https://github.com/leanprover/lean4/pull/12973) Theorems are now opaque in the kernel; use `def` for proof terms that must reduce.
...
- [#13005](https://github.com/leanprover/lean4/pull/13005) Metaprograms calling `compileDecl` directly may need to call `markMeta` first.
- [#12771](https://github.com/leanprover/lean4/pull/12771) `String.Slice.Pos.cast` now requires `s.copy = t.copy` instead of `s = t`.
- [#12435](https://github.com/leanprover/lean4/pull/12435) Changes the signature of `Option.getElem?_inj`.
- [#12708](https://github.com/leanprover/lean4/pull/12708) `PostCond` functions reorder implicit parameters so `α` consistently comes before `ps`.
