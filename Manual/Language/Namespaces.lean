...
Names are organized into hierarchical {deftech}_namespaces_, which are the primary means of organizing APIs in Lean.
Names that contain periods (`'.'`) that aren't inside guillemets are hierarchical names; the periods separate the _components_ of a name.
...
There is a root namespace that contains no name components.
It can be explicitly indicated by beginning a name with `_root_`, which can be necessary in contexts where a name would otherwise be interpreted relative to an ambient namespace (e.g. from a {tech}[section scope]) or local scope.

# Namespaces and Section Scopes

Every {tech}[section scope] has a {deftech}_current namespace_.
Names that are declared within the section scope are added to the current namespace.
If the name has more than one component, then its namespace is nested within the current namespace.
...
Declarations with the {keyword}`protected` modifier are not brought into scope their namespace is opened.

:::example "Current Namespace and Nesting"

 * Explicit root def
 * Nest some namespace decls
...
There are many variations on opening namespaces, providing flexibility while managing the local scope.
...
Dots aren't always namespaces, might be generalized field notation...
