...
open Lean.Parser.Command («inductive» computedField)
...
After declaring an inductive type, its type constructor, constructors, and eliminator are present in the environment.
...
If an inductive type has just one constructor, then this constructor is eligible for _anonymous constructor syntax_.
 ...
 * Projections
 * Inheritance
 * Structure update/constructor syntax
 * `where`-syntax
 * Equivalent `inductive`
:::
...
Every inductive type is equipped with an _eliminator_.
The eliminator is completely determined by the signatures of the type constructor and the constructors.
Eliminators have function types, but they are primitive and are not definable using `fun`.

### Eliminator Types

The eliminator takes the following parameters:
: The inductive type's {tech}[parameters]

  Because parameters are consistent, they can be abstracted over the entire eliminator

: The {deftech}_motive_

  The motive determines the type of an application of the eliminator. The motive is a function whose arguments are the type's indices and an instance of the type with these indices instantiated. The specific universe for the type that the motive determines depends on the inductive type's universe and the specific constructors—see the section on {ref "subsingleton-elimination"}[{tech}[subsingleton] elimination] for details.

: A case for each constructor

  For each constructor, the eliminator expects a function that satisfies the motive for an arbitrary application of the constructor. Each case abstracts over all of the constructor's parameters. If the constructor's parameter's type is the inductive type itself, then the case additionally takes a parameter whose type is the motive applied to that parameter's value.

: The target

  Finally, the eliminator takes an instance of the type as an argument, along with any index values.

The result type of the eliminator is the motive applied to these indices and the target.

:::example "The eliminator for {lean}`Bool`"
{lean}`Bool`'s eliminator {lean}`Bool.rec` has the following parameters:
...
::::example "The eliminator for {lean}`List`"
{lean}`List`'s eliminator {lean}`List.rec` has the following parameters:
...
::::example "The eliminator for {name}`EvenOddList`"
The eliminator {name}`EvenOddList.rec` is very similar to that for `List`.
...
When using a predicate (that is, a function that returns a {lean}`Prop`) for the motive, eliminators express induction.
...
This is reflected in the types of eliminators for inductively defined propositions or predicates.
For these types, the motive may only return another {lean}`Prop` if there's more than one potential proof of the theorem.
...
Its eliminator has the following signature:
```signature
...
Or.rec {a b : Prop} {motive : a ∨ b → Prop}
  ...
  (t : a ∨ b) : motive t
```
The motive's type indicates that it can only be used to eliminate into other propositions.
...
These rules govern the interaction between eliminators and constructors; specifically eliminators that have constructors as their targets.
This form of reduction is called {deftech}_ι-reduction_ (iota reduction){index}[ι-reduction]{index (subterm:="ι (iota)")}[reduction].

When the eliminator's target is a constructor with no recursive parameters, the eliminator application reduces to an application of the constructor's case to the constructor's arguments.
If there are recursive parameters, then these arguments to the case are found by applying the eliminator to the recursive occurrence.

## Well-Formedness Requirements

### Universe Levels

An inductive type's universe must be at least

:::TODO
 * Parameters vs indices and their effect on size
...
In addition to the type constructor, constructors, and eliminators that Lean's core type theory prescribes for inductive types, Lean constructs a number of useful helpers.
First, the equation compiler (which translates recursive functions with pattern matching in to applications of eliminators) makes use of these additional constructs:
 * `recOn` is a version of the eliminator in which the target is prior to the cases for each constructor.
 * `casesOn` is a version of the eliminator in which the target is prior to the cases for each constructor, and recursive arguments do not yield induction hypotheses. It expresses case analysis rather than primitive recursion.
 ...
 * `brecOn` is a version of the eliminator in which `below` is used to provide access to all subtrees, rather than just immediate recursive parameters. It represents strong induction.
...
Recursive functions are compiled as they are in most programming languages, rather than by using the inductive type's eliminator.
Elaborating recursive functions to eliminators serves to provide reliable termination evidence, not executable code.
