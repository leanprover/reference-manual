...
: Recursive functions as fixpoints
  ...
  Taking the function definition as an equation that specifies the behavior of the function, in
  certain cases the existence of a function satisfying this specification can be proven. This stategy can apply even when the function definition is not necessarily terminating on all inputs; hence the term {tech}_partial fixpoint_.
  ...
  As with well-founded recursion, applications of functions defined via partial fixpoint are not definitially equal to their return values.
