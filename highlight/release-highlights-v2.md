...
You will be given a Lean release notes file (e.g. `v4.30.0-v2`) — a `.lean` file with embedded markdown under `Manual/Releases/` in `leanprover/reference-manual`.
...
2. **Fetch and read all PR descriptions** for PRs mentioned in the release notes. Use `gh pr view NNNNN --repo leanprover/lean4 --json body` for each. This is essential because:
...
3. Look at the previous release's highlights to understand the current trajectory of features (e.g., was [`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean) highlighted last time? Is the module system still experimental?).
...
- Significant new tactic capabilities ([`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean), [`simp`](https://github.com/leanprover/reference-manual/blob/main/Manual/Simp.lean), [`bv_decide`](https://github.com/leanprover/reference-manual/blob/main/Manual/BasicTypes/BitVec.lean), etc.)
- Monadic verification / [`mvcgen`](https://github.com/leanprover/reference-manual/blob/main/Manual/VCGen.lean) features — these are a key research direction and should be highlighted when present
...
- **Flagship features** (e.g., a major tactic getting new capabilities like [`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean)): Give these generous space. Use sub-headings (`###`) for each major new capability, include code examples from PR descriptions. The reader should understand what's new and be able to try it. When a feature like [`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean) has multiple distinct new capabilities in one release, each deserves its own sub-section with examples. However, if a feature is being *introduced for the first time*, a brief announcement with a link to its documentation may be better than a deep dive — the reference manual is the right place for comprehensive docs, not the release notes.
...
- **Related breaking changes should be unified into a single narrative.** When multiple PRs address the same underlying issue (e.g., transparency handling changes touching `isDefEq`, `@[implicit_reducible]`, [`simp +instances`](https://github.com/leanprover/reference-manual/blob/main/Manual/Simp.lean), and `inferInstanceAs`), present them as one coherent story with a migration guide, NOT as separate highlights. The reader needs to understand the whole picture, not piece it together from fragments.
...
- Incremental improvements to a feature that was already highlighted in a previous release, unless there's a qualitative leap (e.g., don't re-highlight [`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean) every release just because it got faster; DO highlight when [`grind`](https://github.com/leanprover/reference-manual/blob/main/Manual/Grind.lean) gets a fundamentally new capability like interactive mode)
