...
slug := "vcgen"
summary := inlines!"A demonstration of how to use Lean's verification condition generator to conveniently and compositionally prove properties of programs."
...
```lean
-- A future 4.36 release adds these annotations to the library.
attribute [local grind norm]
  Lean.Order.ofProp_prop_eq Lean.Order.meet_prop_eq_and
  Lean.Order.join_prop_eq_or Lean.Order.iSup_prop_eq_exists
...
```
The {name}`WP` instance for {name}`Result` specifies the assertion type {lean}`Prop` because there are no state-like effects, and the exception postcondition type {lean}`Error → Prop` because there is a single exception of type {lean}`Error`.
The {name}`WP` instance translates programs in {lean}`Result α` to predicate transformers in {lean}`PredTrans Prop (Error → Prop) α`.
...
::::
```lean
instance : WP (Result α) α Prop (Error → Prop) where
  wpTrans
    | .ok v => ⟨fun post _ => post v⟩
    | .fail e => ⟨fun _ epost => epost e⟩
    | .div => ⟨fun _ _ => False⟩
...
It is a priority of {tactic}`vcgen` to break down programs into {tech (remote := "reference")}[verification conditions] that are straightforward to understand.
