...
The {tech (key := "mvcgen weakest precondition") (remote := "reference")}_weakest precondition_ $`\textbf{wp}⟦\mathit{prog}⟧(Q)` of a program $`\mathit{prog}` and postcondition $`Q` is a precondition for which $`\mathit{prog}` ensures the postcondition $`Q` and is implied by all other such preconditions.
...
This lemma states that facts about the result of running a computation in the {name}`Id` monad that terminates normally (`Id` computations never throw exceptions) can be proved by showing that the {tech (key := "mvcgen weakest precondition") (remote := "reference")}[weakest precondition] that ensures the desired result is true.
Next, the proof uses {tactic}`mvcgen` to replace the formulation in terms of weakest preconditions with a set of {tech (key := "mvcgen verification conditions") (remote := "reference")}[verification conditions].

While {tactic}`mvcgen` is mostly automatic, it does require an invariant for the loop.
A {tech (key := "mvcgen loop invariant") (remote := "reference")}_loop invariant_ is a statement that is both assumed and guaranteed by the body of the loop; if it is true when the loop begins, then it will be true when the loop terminates.
...
Whenever referring to cases, only this prefix should be used; the suffix is merely a hint to the user of where that particular {tech (key := "mvcgen VC") (remote := "reference")}[VC] came from.
For example:

* `vc1.step` conveys that this {tech (key := "mvcgen VC") (remote := "reference")}[VC] proves the inductive step for the loop
...
Therefore, the only difference is in the {tech (key := "mvcgen loop invariant") (remote := "reference")}[loop invariant].
...
A {tech (key := "mvcgen Hoare triple") (remote := "reference")}_Hoare triple_ consists of a precondition, a statement, and a postcondition; it asserts that if the precondition holds, then the postcondition holds after running the statement.
...
:::paragraph
Hoare triples are defined in terms of a logic of stateful predicates plus a {tech (key := "mvcgen weakest precondition") (remote := "reference")}[weakest precondition] semantics {lean}`wp⟦prog⟧` that translates monadic programs into this logic.
...
A more compositional way is to develop individual {tech (key := "mvcgen specification lemmas") (remote := "reference")}_specification lemmas_ for each monadic function.
A specification lemma is a Hoare triple that is automatically used during {tech (key := "mvcgen verification condition") (remote := "reference")}[verification condition] generation to obtain the pre- and postconditions of each statement in a {keywordOf Lean.Parser.Term.do}`do`-block.
...
This is referred to as {deftech (key := "mvcgen framing")}_framing_ hypotheses (cf. the {tactic}`mpure` and {tactic}`mframe` tactics).
Hypotheses in the Lean context are part of the immutable {deftech (key := "mvcgen frame")}_frame_ of the stateful logic, because in contrast to stateful hypotheses they survive the rule of consequence.
...
: The {tech (key := "mvcgen total correctness interpretation") (remote := "reference")}_total correctness interpretation_

  {lean}`⦃P⦄ prog ⦃⇓ r => Q' r⦄` asserts that, given {lean}`P` holds, then {lean}`prog` terminates _and_ {lean}`Q'` holds for the result.

: The {tech (key := "mvcgen partial correctness interpretation") (remote := "reference")}_partial correctness interpretation_
...
Rather, {tactic}`mvcgen` relies on instances of the {name}`WP` and {name}`WPMonad` type class and user-provided specifications to generate {tech (key := "mvcgen verification conditions") (remote := "reference")}[verification conditions].
...
It is a priority of {tactic}`mvcgen` to break down monadic programs into {tech (key := "mvcgen verification conditions") (remote := "reference")}[verification conditions] that are straightforward to understand.
